...
if "arabica_test_db" in stdout_data:
    # Drop the database
    execute_shell("%sdropdb -h localhost %s arabica_test_db" % (login_data, jenkins_flag))

# Create the new database
execute_shell("%screatedb -h localhost %s arabica_test_db" % (login_data, jenkins_flag))
# execute_shell("%spsql -h localhost %s arabica_test_db < evolutions/default/1.sql" % (login_data, jenkins_flag))
